...
Da Fase 0, recuperamos a noção de sistema como conjunto de componentes relacionados que opera com algum propósito. Recuperamos também que uma interface é um ponto de contato entre partes, e não apenas uma tela. Essas ideias ajudam a reconhecer que dividir não significa isolar completamente. Entradas, saídas, informação, materiais, decisões e mudanças de estado podem atravessar a fronteira escolhida.
...
- **critério declarado:** sabemos se dividimos por funções, objetos de atenção, etapas ou outro princípio;
...
Uma parte não é automaticamente um subproblema. “Documento da solicitação” pode ser um objeto de atenção, enquanto “reduzir documentos inválidos sem excluir solicitantes” pode ser um subproblema. Uma função não é automaticamente uma tarefa: “validar elegibilidade” expressa uma contribuição; “analista confere o comprovante” descreve uma ação mais localizada. Uma etapa não é necessariamente um componente permanente, e responsabilidade não é sinônimo de departamento.
...
Também é possível ter uma sequência sem decomposição adequada: “receber, analisar, concluir” informa três momentos, porém não revela quais dados orientam a análise, que decisão autoriza a conclusão, o que ocorre diante de pendência nem como uma restrição de privacidade atravessa as etapas. E é possível decompor sem ordenar: uma análise por objetos pode considerar solicitação, documento, decisão e comunicação sem afirmar uma ordem única.
...
Uma boa decomposição funcional ajuda a verificar responsabilidades, entradas e resultados parciais. Ela não determina ordem completa, causalidade ou partes técnicas. Funções amplas, duplicadas ou vagas podem esconder dados; copiar a solução ou os departamentos atuais cristaliza seus defeitos. “Ler o campo” e “clicar em confirmar” podem ser detalhes prematuros.
...
## Decomposição por dados e objetos de atenção

Esta decomposição pergunta: **sobre quais informações, registros, documentos, itens ou objetos do domínio precisamos raciocinar separadamente?** Em uma concessão de benefício, exemplos podem ser solicitação, pessoa solicitante, documento apresentado, regra vigente, análise, decisão, comunicação e contestação.

O objetivo aqui não é construir um modelo técnico de dados. Não definiremos campos, chaves, tabelas, formatos de armazenamento nem estruturas de software. “Objeto” significa apenas algo do domínio cuja identidade e condição importam para o problema. A pergunta é analítica: onde a informação nasce, quem a usa, quando muda, que regra a restringe e que inconsistência afetaria outras partes?

Esse critério é útil quando um mesmo registro percorre várias funções ou quando nomes iguais escondem significados distintos. “Data da solicitação”, “data de recebimento completo” e “data da decisão” não são intercambiáveis. A divisão por objetos pode revelar que uma métrica de prazo depende de qual marco foi registrado, sem concluir que o registro explica causalmente o atraso.

O risco é produzir um inventário de substantivos, copiar tabelas existentes, confundir arquivo com realidade, duplicar conceitos ou ignorar processos e decisões. Fronteiras não são óbvias. Registre papel e interação: “documento — sustenta verificação; sua condição pode motivar complementação” é melhor que “documentos”.

## Decomposição por etapas e eventos

A decomposição por etapas acompanha **períodos reconhecíveis do percurso**, enquanto eventos registram **ocorrências que iniciam, alteram ou encerram uma condição relevante**. No caso do benefício, etapas possíveis são entrada, preparação da análise, decisão, comunicação e pós-decisão. Eventos possíveis incluem recebimento da solicitação, identificação de pendência, chegada de complemento, registro de decisão e apresentação de contestação.
...
| dados/objetos | sobre quais informações? objetos do domínio | produção, uso e mudança | localizar divergências | copiar estruturas existentes | investigar significados |
...
- **informação:** a análise precisa conhecer documentos recebidos e regras vigentes;
- **ordem:** uma decisão individual só pode ser comunicada depois de autorizada;
- **recurso:** duas partes disputam a mesma equipe, sala ou janela operacional;
- **decisão:** o pagamento depende de decisão válida, não apenas de recomendação;
- **regra:** análise e contestação precisam interpretar a mesma regra aplicável;
...
Também não confunda dependência com causa. “A análise recebe o registro de documentos” não afirma que o registro causou um atraso. Setas entre partes precisam de rótulo; uma seta muda de significado quando representa “fornece informação”, “requer autorização”, “restringe” ou “ocorre antes”.

## Interfaces e mapa inicial de dependências

Nesta aula, **interface** é o ponto ou a relação de interação entre fronteiras analíticas. Ela pode envolver informação, material, decisão, regra ou mudança de estado. Uma tela é apenas uma possível manifestação futura; interface não significa necessariamente tela, API ou especificação técnica.
...
Exemplo: “verificação documental → análise: condição dos documentos e pendências encerradas; a análise precisa distinguir ‘recebido’ de ‘legível e disponível’; se essa distinção faltar, casos podem ser analisados com base incompleta”. O registro é analítico. Não define formato, protocolo ou implementação.

Um mapa simples pode ser uma tabela:

| Origem → destino | Relação ou conteúdo | Condição | Consequência da ausência |
|---|---|---|---|
| análise → decisão | conclusão e regras aplicadas | requer papel autorizado | não há decisão válida |
| decisão → comunicação | resultado, fundamento e prazo | após autorização | interessado não conhece resultado ou direito |
| contestação → análise | alegação e eventual nova evidência | se contestação admissível | revisão pode usar base incompleta |
...
Como modelo didático simplificado, use conjuntos; nem toda decomposição real cabe integralmente neles. Seja `W` o conjunto dos aspectos relevantes segundo um único critério e sejam `P₁, P₂, ..., Pₙ` os escopos das partes, com `Pᵢ ⊆ W`. Há cobertura quando:

`P₁ ∪ P₂ ∪ ... ∪ Pₙ = W`

O símbolo `⊆` significa “está contido em”; `∪` significa união, isto é, tudo o que aparece em pelo menos uma parte; `=` afirma que essa reunião corresponde ao conjunto de aspectos relevantes definido como `W`. A igualdade não é selo automático de qualidade: depende de termos claros e de verificação contra o caso.
...
Não reúna na mesma igualdade partes funcionais, objetos e etapas como se fossem conjuntos equivalentes. Cada critério recorta o todo de forma diferente. A comparação entre critérios é argumentativa: verificamos o que cada visão torna visível e onde uma complementa a outra.

## Sobreposição, lacunas e elementos transversais

Duas partes se sobrepõem quando seus escopos compartilham algum aspecto. Em notação:
...
Uma **partição estrita** de `W` exige cobertura, partes não vazias e partes disjuntas duas a duas: para qualquer `i ≠ j`, `Pᵢ ∩ Pⱼ = ∅`. Essa estrutura é útil em classificações que exigem um único grupo por item. Decomposições sociotécnicas, porém, não precisam ser partições estritas. Privacidade pode restringir recebimento, análise, decisão e comunicação; a regra vigente pode ser consultada por análise e contestação. Esse alcance transversal é legítimo quando a responsabilidade e o efeito em cada parte estão claros.

Sobreposição torna-se **duplicidade problemática** quando duas partes executam ou decidem a mesma coisa sem coordenação, produzem versões incompatíveis ou deixam incerto quem responde pelo resultado. Se análise e decisão alteram independentemente a interpretação da regra, há risco de conclusões divergentes. Se ambas precisam consultar a mesma regra controlada, há sobreposição legítima de dependência.
...
**Visão por dados e objetos de atenção:** pedido, linha, produto, centro, posição registrada, quantidade física observada, disponibilidade ainda a definir, movimento, ocorrência, regra ou proteção, avaliação, autorização, resposta e indicador posterior. Não é esquema técnico. Posição, quantidade física e disponibilidade não são equivalentes. Os 29 históricos incompletos limitam afirmações sobre movimentos; não provam causa para as 192 ocorrências.

**Visão por etapas e eventos:** demanda registrada; preparação das informações; avaliação de disponibilidade; primeira separação; registro do resultado; eventual divergência; consolidação; avaliação; eventual decisão autorizada; acompanhamento de ciclos e proteções. Nem todo caso tem divergência, verificações podem ser paralelas e o quadro não prescreve fluxo novo.

| Critério e pergunta | Relação principal | Vantagem e limite | Lacuna possível | Sobreposição possível |
|---|---|---|---|---|
| função — o que precisa ocorrer? | contribuições e resultados | mostra responsabilidades; deixa percursos menos visíveis | tratamento dos 29 históricos incompletos | avaliação e autorização |
| objetos — que informação importa? | produção, mudança e uso | expõe divergências; deixa ordem em segundo plano | pessoa afetada | regra e indicador compartilhados |
| etapas — em que momentos? | precedência, retorno e espera | localiza interrupções; pode esconder funções | autoridade para reabrir análise | verificações paralelas |

Mapa textual selecionado:

- demanda registrada → avaliação de disponibilidade: **fornece informação** sobre produto, centro e quantidade;
- movimentos → posição registrada: **atualizam estado** quando completos e interpretados pelo mesmo critério;
- posição e condição física → avaliação de disponibilidade: **fornecem informações candidatas**, cuja definição permanece aberta;
- avaliação de disponibilidade → separação: **informa** a condição considerada no momento;
- separação → medição: **produz registro** do atendimento da linha;
- divergências → avaliação: **fornecem evidência** com limites declarados;
- meta candidata → avaliação: **oferece referência provisória**, sujeita a validação;
- proteções → avaliação: **limitam respostas aceitáveis** quando legitimadas;
- avaliação → eventual resposta: **requer autorização**, ainda não identificada;
- eventual resposta → acompanhamento: **produz resultados** para comparar ciclos e efeitos;
...
Antes do comentário, anote: critério de cada proposta; cobertura; uma lacuna; uma dependência; uma sobreposição; e como recomporia o resultado. **Comentário:** A mistura objeto, canal, condição, papel e interessado, sem critério. B declara funções, mas pode precisar separar registro e análise da contestação. Proteger dados atravessa todas; analisar depende da obrigação, da cobrança e das evidências de pagamento. “Atendente” é papel, não parte equivalente.

Se a pergunta mudar para localizar divergências de estado, objetos como obrigação, cobrança emitida, pagamento informado, pagamento conciliado, contestação e comunicação ganham utilidade. A visão funcional mostra contribuições; a de objetos, significados. Fundir pagamento informado e conciliado pode ocultar uma divergência. Nenhuma visão explica causalmente o atraso nem escolhe automação.
...
- **“Tabelas existentes revelam os objetos.”** Estruturas atuais podem preservar conceitos inadequados.
...
- **“Interfaces são telas.”** Elas são interações de informação, material, decisão, regra ou estado.
...
Resolva sem consultar uma resposta pronta. Nas questões abertas, declare o critério usado e sustente cada escolha com elementos do enunciado.

1. **Objetiva — definição.** Qual opção descreve melhor decomposição nesta aula?
   - A) Relacionar todos os elementos lembrados e distribuí-los entre equipes para acelerar a execução.
   - B) Dividir um todo formulado segundo propósito e critério, preservar dependências e verificar sua recomposição.
   - C) Ordenar atividades do início ao fim e detalhá-las até que cada uma pertença a uma pessoa.
   - D) Escolher antecipadamente uma solução e separar os elementos técnicos necessários para construí-la.
...
2. **Objetiva — dependência e ordem.** Duas verificações foram agendadas para manhã e tarde; ambas consultam a mesma regra, mas nenhuma usa o resultado da outra. A leitura mais precisa é:
   ...
   - D) a regra é apenas uma parte da sequência e não precisa ser registrada como elemento transversal.

3. **Objetiva — sobreposição.** Privacidade restringe recebimento, análise e comunicação. Isso indica que:
   - A) a decomposição falhou, pois uma restrição só pode pertencer a uma parte;
   - B) as três partes devem ser unidas para formar uma partição estrita;
   - C) pode haver sobreposição legítima, desde que o efeito e a responsabilidade sejam explícitos em cada parte;
   - D) privacidade deve ficar fora da decomposição por não produzir um resultado parcial.
...
5. **Dissertativa — distinções.** Compare parte, subproblema, função, tarefa e etapa usando um único caso. Não use cinco exemplos desconectados.

6. **Dissertativa — hierarquia e parada.** Crie dois níveis funcionais para “tratar contestação” e justifique onde parou. Indique um detalhe que seria prematuro no propósito atual.

7. **Dissertativa — ordem e dependência.** Produza um exemplo de ordem sem dependência e outro de dependência sem ordem fixa. Para cada um, explique o teste usado.

8. **Dissertativa — recomposição.** Explique como uma melhoria local pode piorar o todo. Relacione uma métrica local, um critério global e uma proteção.

9. **Análise de caso — lista arbitrária.** Reescreva “cadastro, gerente, documento, três dias, análise, tela” como decomposição funcional coerente de uma solicitação. Registre o que foi removido, reposicionado ou renomeado.

10. **Análise de caso — critérios diferentes.** Uma escola quer reduzir matrículas que exigem correção. Produza uma decomposição funcional e outra por objetos de atenção, com quatro a seis partes cada. Compare o que cada uma evidencia.

11. **Análise de caso — estoque.** Escolha uma função ampla do caso progressivo, decomponha-a em três a cinco partes e justifique a granularidade. Preserve os limites de evidência e não proponha solução.

12. **Análise matemática — cobertura e interseção.** Para `W = {receber, verificar, decidir, comunicar, contestar}`, considere `P₁ = {receber, verificar}`, `P₂ = {verificar, decidir}`, `P₃ = {comunicar}`. Calcule a união, identifique uma interseção não vazia, indique a lacuna e diga se há partição estrita. Depois proponha a menor correção explícita, sem usar “outros”.

13. **Contraexemplo.** Construa um contraexemplo para a afirmação “se todas as tarefas foram atribuídas, o problema foi bem decomposto”. Mostre pelo menos uma dependência ou propriedade global perdida.

14. **Análise de caso — mapa.** Para uma contestação de cobrança, registre seis partes e sete dependências rotuladas. Inclua uma dependência condicional, uma restrição transversal e a consequência de uma interface ambígua.

15. **Revisão.** Receba esta divisão: “entrada, análise, gerente, documento, saída”. Faça duas revisões entre critério, fronteira, granularidade ou dependência. Para cada mudança, declare a razão e o efeito sobre cobertura ou recomposição.

16. **Integração — Projeto Parcial 1.** Audite as três decomposições do estoque. Identifique uma lacuna possível, uma sobreposição legítima, um risco de duplicidade e uma questão aberta que pode alterar a estrutura. Conclua se o artefato está suficiente para avançar à modelagem da Aula 04, sem criar o modelo.
...
5. **Função:** contribuição ou transformação necessária.
...
11. **Decomposição por dados:** divisão por informações ou objetos tratados.
...
19. **Interface:** interação na fronteira entre partes.
...
A Aula 04 perguntará quais detalhes precisam aparecer ou podem ser omitidos, para qual propósito, em qual nível e com quais perdas. Esta aula não antecipa entidade, atributo, teoria de modelos, representação formal, fluxograma, pseudocódigo ou partes de software.
...
- **Extensão:** 6.783 palavras, 478 linhas, 27 seções H2, sem subseções.
- **Atividades e casos:** 16 exercícios — 3 objetivos, 5 dissertativos e 8 de análise ou integração —; 3 casos desenvolvidos, além de casos curtos; 6 fontes.
- **Matemática:** subconjunto, união, interseção, vazio, cobertura, sobreposição, partição, percentuais e desigualdade.
- **Aula sem a prática:** leitura, 1h45–2h15; recuperação, 25–35 min; exercícios, 3h20–4h30; revisão, 30–45 min; total, **6h00–8h05**.
- **Prática e total:** prática essencial, **4h05–5h20**; aprofundada, **5h20–7h00**. Aula completa com prática essencial, **10h05–13h25**; com aprofundamento, **11h20–15h05**.
