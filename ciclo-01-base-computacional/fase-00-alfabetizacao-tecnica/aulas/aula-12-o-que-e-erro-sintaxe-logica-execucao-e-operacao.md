...
## Erro, falha, defeito e comportamento inesperado

Na prática cotidiana, as palavras erro, falha e defeito costumam ser usadas como sinônimos. Para a Fase 0, não é necessário entrar em engenharia de confiabilidade formal, mas vale separar algumas ideias. Erro pode ser entendido como a ação, condição ou resultado que se desvia do esperado. Defeito é uma causa interna ou condição existente que pode levar a problema, como uma regra implementada de forma errada. Falha é a manifestação percebida quando algo não funciona como deveria.

Comportamento inesperado é aquilo que surpreende quem usa, opera ou analisa o sistema. Ele pode ser erro real, regra desconhecida, expectativa equivocada ou efeito de configuração. Resultado incorreto é uma saída errada, mesmo que o sistema tenha executado sem parar. Incidente é um evento que afeta ou ameaça afetar a operação, como indisponibilidade, processamento indevido ou exposição de dado.

Essas distinções ajudam porque problemas têm camadas. Um defeito pode existir por meses sem ser percebido. Uma falha pode aparecer apenas quando certo dado entra. Um comportamento inesperado pode revelar erro na regra, na documentação ou na expectativa do usuário. Um resultado incorreto pode ser mais perigoso que uma parada explícita, porque parece normal até alguém conferir.
...
Erro lógico é problema no raciocínio, na regra, na condição, no cálculo, na interpretação do processo ou na formulação da solução. Ele pode existir mesmo quando a escrita formal está correta. O sistema aceita a instrução, executa normalmente e ainda assim produz resultado errado porque a ideia implementada está equivocada.
...
Erro de execução é problema que aparece quando o programa, rotina ou sistema está sendo executado. Nesse momento, não se trata apenas de compreender a escrita ou a lógica planejada. O programa está tentando agir em um ambiente real ou controlado, usando arquivos, dados, memória, permissões, configurações, dependências e recursos.

Um erro de execução pode ocorrer por arquivo ausente, permissão negada, divisão por zero, dado incompatível, recurso indisponível, conexão interrompida, falta de memória, configuração errada, dependência ausente, caminho inválido ou situação não tratada. A Aula 09 preparou a ideia de execução; a Aula 10 mostrou o papel de arquivos; a Aula 11 explicou que o ambiente influencia o comportamento.
...
Erro é uma diferença entre o esperado e o ocorrido, ou uma violação de regra, formato, condição, comportamento ou resultado. Em tecnologia, essa ideia precisa ser classificada. Erro de sintaxe envolve forma de escrita. Erro lógico envolve raciocínio, regra ou cálculo. Erro de execução aparece quando o programa está rodando e encontra uma condição problemática. Erro operacional surge no uso real de processos, rotinas, ambientes e procedimentos.

Também existem erros de ambiente, dados e regra de negócio. Um sistema pode estar tecnicamente funcionando e ainda produzir resultado errado. Por isso, análise de erro precisa observar entrada, processamento, saída, arquivos, ambiente, permissões, configurações, versões, operação, mensagens, logs e evidências.
...
- Erro: diferença entre o esperado e o ocorrido, ou violação de regra, condição, formato, comportamento ou resultado.
- Defeito: condição interna que pode causar problema, como regra implementada de forma incorreta.
- Falha: manifestação percebida quando algo não funciona como deveria.
- Incidente: evento que afeta ou ameaça afetar a operação de um sistema ou processo.
...
- Erro lógico: problema no raciocínio, cálculo, condição, regra ou formulação da solução.
- Erro de execução: problema que aparece enquanto programa, rotina ou sistema está executando.
